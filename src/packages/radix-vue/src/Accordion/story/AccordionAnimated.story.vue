...
import { AccordionContent, AccordionHeader, AccordionItem, AccordionRoot, AccordionTrigger } from '../';
...
<template>
  <Story title="Accordion/Animated" :layout="{ type: 'grid', width: '50%' }" auto-props-disabled>
    <Variant title="Closed by default">
      <template #controls> modelValue: <input v-model="hasDynamicContent" type="checkbox" /> </template>

      <AccordionRoot class="w-[300px] rounded-md bg-[--line-color] shadow-lg" type="single">
        <AccordionItem v-for="value in values" :key="value" class="accordion-item" :value="value">
    ...
    <Variant title="Open by default">
      ...
      <AccordionRoot class="w-[300px] rounded-md bg-[--line-color] shadow-lg" type="single" default-value="One">
        <AccordionItem v-for="value in values" :key="value" class="accordion-item" :value="value">
          <AccordionHeader class="flex">
            <AccordionTrigger class="accordion-trigger">
              {{ value }}
    ...
    <Variant title="Animated Controlled">
      <AccordionRoot v-model="selectedValue" class="w-[300px] rounded-md bg-[--line-color] shadow-lg" type="multiple">
        ...
        <AccordionItem class="accordion-item" value="Three">
          <AccordionHeader class="flex">
            <AccordionTrigger class="accordion-trigger"> Three </AccordionTrigger>
          </AccordionHeader>
          <AccordionContent
            class="accordion-animated-content data-[state=open]:animate-slideDown data-[state=closed]:animate-slideUp"
          >
