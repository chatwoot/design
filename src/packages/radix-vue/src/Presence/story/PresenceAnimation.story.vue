...
<template>
  <Story group="utilities" title="Presence/Animation" :layout="{ type: 'grid', width: '33%' }" auto-props-disabled>
    ...
    <Variant title="With Open & Close Animation">
      <Animation class="data-[state=open]:animate-fadeIn data-[state=closed]:animate-fadeOut" />
    </Variant>
    ...
    <Variant title="With Deferred Mount Animation">
      <div>
        <p class="text-xs mb-4">
          Deferred animation should unmount correctly when toggled. Content will flash briefly while we wait for
          animation to be applied.
        </p>
        <Toggle v-model:open="open" :node="node" />
        <Presence :present="open">
          <div ref="node" :class="animate ? 'animate-fadeIn' : undefined">Content</div>
        </Presence>
      </div>
    </Variant>
  </Story>
